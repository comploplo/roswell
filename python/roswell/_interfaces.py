...
(see ``ros_type_resolve``): Python's only job is to hand the loader the list of
root directories to search. Roots come, in priority order, from the caller's
``type_paths``, the ``ROSWELL_TYPE_PATH`` and ``AMENT_PREFIX_PATH`` environment
...
def search_roots(type_paths: Sequence = ()) -> list[str]:
    ...
    roots += _split_env("ROSWELL_TYPE_PATH")
    roots += _split_env("AMENT_PREFIX_PATH")
    roots += _split_env("ROSWELL_INTERFACES")  # legacy bundled-tree override
