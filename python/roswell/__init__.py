"""roswell — pip-installable Python that speaks ROS2 with zero ROS installation.

A thin, asyncio-native client over the roswell Rust runtime (RTPS via RustDDS).
...
class QosProfile:
    ...
    def preset(cls, name: str) -> "QosProfile":
        ...
        q = lib.ros_qos_preset(name.encode())
        return cls(
...
class Publisher:
    ...
    def __repr__(self) -> str:
        return f"<roswell.Publisher topic={self.topic!r} type={self.type.root!r}>"

    def publish(self, msg: Message) -> None:
        check_rc(lib.ros_publish(self._handle, msg.handle), "publish")
        _emit_qos_warnings(take_json(lib.ros_publisher_events(self._handle)))
    ...
    def close(self) -> None:
        if self._handle:
            lib.ros_publisher_free(self._handle)
            self._handle = None
...
class Subscriber:
    ...
    def __repr__(self) -> str:
        return f"<roswell.Subscriber topic={self.topic!r} type={self.type.root!r}>"
    ...
    def take(self) -> Optional[Message]:
        ...
        msg = self.type.alloc()
        r = lib.ros_take(self._handle, msg.handle)
        _emit_qos_warnings(take_json(lib.ros_subscriber_events(self._handle)))
        if r == 1:
            return msg
        if r < 0:
    ...
    def messages(self, timeout: float = 1.0) -> Iterator[Message]:
        ...
        while True:
            ...
            if msg is not None:
                yield msg
                continue
            if lib.ros_wait(handles, 1, int(timeout * 1000)) < 0:
                return
    ...
    def close(self) -> None:
        ...
        with self._node._lock:
            if self._registered:
                ...
                self._registered = False
            if self._handle:
                lib.ros_subscriber_free(self._handle)
                self._handle = None
...
class Client:
    ...
    def __repr__(self) -> str:
        return f"<roswell.Client name={self.name!r}>"
    ...
    def call_sync(self, req: Message, timeout: float = 5.0) -> Message:
        ...
        r = lib.ros_call(self._handle, req.handle, resp.handle, int(timeout * 1000))
        if r == 1:
            return resp
        if r == 0:
            raise RoswellTimeout(f"service call timed out after {timeout}s")
    ...
    def close(self) -> None:
        if self._handle:
            lib.ros_client_free(self._handle)
            self._handle = None
...
class Service:
    ...
    def __repr__(self) -> str:
        return f"<roswell.Service name={self.name!r}>"
    ...
    def _serve(self) -> None:
        ...
        while not self._stop.is_set():
            r = lib.ros_service_take_request(self._handle, req_buf.handle, byref(token))
            if r == 1:
                resp = self._invoke(req_buf)
                lib.ros_service_send_reply(self._handle, token.value, resp.handle)
            elif r < 0:
    ...
    def close(self) -> None:
        ...
        if self._handle:
            lib.ros_service_free(self._handle)
            self._handle = None
...
#: The eight component types of an action, in the order ros_action_load returns.
_ACTION_TYPE_NAMES = (
    "goal",
    "result",
    "feedback",
...
class ActionTypes:
    ...
    def __init__(self, node: "Node", action_type):
        ...
        if lib.ros_action_load(self.action_type.encode(), arr, n, out) != 0:
            raise RoswellError(f"load_action({action_type}): {_last_error()}")
        for name, handle in zip(_ACTION_TYPE_NAMES, out):
            setattr(self, name, MessageType(handle))

    def __repr__(self) -> str:
        return f"<roswell.ActionTypes {self.action_type!r}>"
...
class ActionClient:
    ...
    def __init__(self, node: "Node", handle: int, action_name: str, action_type: str):
        ...
        self.goal_type = MessageType(lib.ros_action_goal_type(handle))
        self.result_type = MessageType(lib.ros_action_result_type(handle))
        self.feedback_type = MessageType(lib.ros_action_feedback_type(handle))

    def __repr__(self) -> str:
        return f"<roswell.ActionClient name={self.name!r} type={self.action_type!r}>"
    ...
    def server_is_ready(self) -> bool:
        ...
        r = lib.ros_action_server_ready(self._handle)
        if r < 0:
    ...
    def get_result_sync(self, goal_id: bytes, timeout: float = 5.0) -> Tuple[int, Message]:
        ...
        result.close()
        if r == 0:
            raise RoswellTimeout(f"get_result timed out after {timeout}s")
    ...
    def poll_feedback(self) -> Optional[Feedback]:
        ...
        goal_id = (c_uint8 * _GOAL_ID_LEN)()
        r = lib.ros_action_poll_feedback(self._handle, fb.handle, goal_id)
        if r == 1:
    ...
    def cancel_sync(self, goal_id: Optional[bytes] = None, timeout: float = 5.0) -> int:
        ...
        code = c_int8()
        r = lib.ros_action_cancel_goal(
            self._handle, _goal_id_buf(goal_id), int(timeout * 1000), byref(code)
        )
        if r == 1:
            return code.value
        if r == 0:
            raise RoswellTimeout(f"cancel_goal timed out after {timeout}s")
    ...
    def close(self) -> None:
        if self._handle:
            lib.ros_action_client_free(self._handle)
            self._handle = None
...
class TfBuffer:
    ...
    def __repr__(self) -> str:
        return f"<roswell.TfBuffer node={self._node.name!r}>"
    ...
    def lookup_transform(
        ...
        :class:`RoswellError` when the transform is not (yet) available.
        """
        ...
        raise RoswellError(f"lookup_transform({target}, {source}): {_last_error()}")
...
class ServerGoalHandle:
    ...
    def __repr__(self) -> str:
        return f"<roswell.ServerGoalHandle goal_id={self.goal_id.hex()}>"
    ...
    def publish_feedback(self, msg: Message) -> None:
        ...
        check_rc(
            lib.ros_action_server_publish_feedback(
    ...
    def is_cancel_requested(self) -> bool:
        ...
        r = lib.ros_action_server_cancel_requested(
...
class ActionServer:
    ...
    def __init__(
        ...
        self.name = name
        self.action_type = action_type
        self.goal_type = MessageType(lib.ros_action_server_goal_type(handle))
        self.result_type = MessageType(lib.ros_action_server_result_type(handle))
        self.feedback_type = MessageType(lib.ros_action_server_feedback_type(handle))
    ...
    def __repr__(self) -> str:
        return f"<roswell.ActionServer name={self.name!r} type={self.action_type!r}>"
    ...
    def _serve(self) -> None:
        goal_id = (c_uint8 * _GOAL_ID_LEN)()
        while not self._stop.is_set():
            lib.ros_action_server_spin(self._handle)
            goal = self.goal_type.alloc()
            r = lib.ros_action_server_take_goal(self._handle, goal.handle, goal_id)
            if r != 1:
                goal.close()
                self._stop.wait(0.005)
                continue
    ...
    def close(self) -> None:
        ...
        if self._thread.is_alive():
            self._thread.join(timeout=1.0)
        if self._handle:
            lib.ros_action_server_free(self._handle)
            self._handle = None
...
def _set_param_array(server: int, name: str, value) -> None:
    ...
    if isinstance(value, (bytes, bytearray)):
        ...
        rc = lib.ros_param_set_array(server, name.encode(), PARAM_BYTE_ARRAY, buf, len(raw))
    else:
        items = list(value)
        if not items:
            raise TypeError(
        ...
        if all(isinstance(v, bool) for v in items):
            ...
            rc = lib.ros_param_set_string_array(server, name.encode(), arr, len(items))
...
class Timer:
    ...
    def __repr__(self) -> str:
        ...
        return f"<roswell.Timer period={self.period} {state}>"
...
class Node:
    ...
    with :func:`ros_wait` and dispatches into their asyncio loops — never one
    ...
    def __init__(
        ...
        self._ctx = check_ptr(lib.ros_init(domain), "ros_init")
        lib.ros_node(self._ctx, name.encode(), namespace.encode())
    ...
    def __repr__(self) -> str:
        ...
        return f"<roswell.Node name={self.name!r} {state}>"
    ...
    def load_type(self, path, deps: Sequence = ()) -> MessageType:
        ...
        against the node's search roots (``type_paths``, ``ROSWELL_TYPE_PATH``,
        ...
        if deps or os.path.exists(str(path)):
            ...
            handle = lib.ros_type_load(str(path).encode(), arr, n)
        else:
            arr, n = _paths(self._search_roots())
            handle = lib.ros_type_resolve(str(path).encode(), arr, n)
    ...
    def load_service(self, path, deps: Sequence = ()) -> tuple[MessageType, MessageType]:
        ...
        if deps or os.path.exists(str(path)):
            arr, n = _paths(list(deps))
            rc = lib.ros_type_load_srv(str(path).encode(), arr, n, byref(req), byref(resp))
        else:
            arr, n = _paths(self._search_roots())
            rc = lib.ros_type_resolve_srv(str(path).encode(), arr, n, byref(req), byref(resp))
        if rc != 0:
            raise RoswellError(f"load_service({path}): {_last_error()}")
    ...
    def tf_buffer(self) -> "TfBuffer":
        ...
        return TfBuffer(self, check_ptr(lib.ros_tf_buffer(self._ctx), "tf_buffer"))
    ...
    def publisher(
        ...
        handle = lib.ros_publisher(self._ctx, topic.encode(), mtype._handle, ptr)
    ...
    def subscribe(
        ...
        arg = _qos_arg(qos)
        ptr = arg[0] if arg else None
        handle = lib.ros_subscriber(self._ctx, topic.encode(), mtype._handle, ptr)
        return Subscriber(self, check_ptr(handle, f"subscribe({topic})"), mtype, topic)

    def client(
        self, name: str, req_t: MessageType, resp_t: MessageType
    ) -> Client:
        handle = lib.ros_client(self._ctx, name.encode(), req_t._handle, resp_t._handle)
    ...
    def serve(
        ...
        handle = lib.ros_service(self._ctx, name.encode(), req_t._handle, resp_t._handle)
    ...
    def graph(self, listen_ms: int = 500) -> dict:
        ...
        return json.loads(take_json(lib.ros_graph_json(self._ctx, listen_ms)))
    ...
    def _params(self) -> int:
        ...
        if self._param_server is None:
            handle = lib.ros_param_server(self._ctx, self.name.encode())
    ...
    def _wait_loop(self) -> None:
        while not self._stop.is_set():
            ...
            idx = lib.ros_wait(handles, len(entries), 100)
            if idx >= 0:
            ...
            for sub, _loop, _q in entries:
                with self._lock:
                    if id(sub) in self._async and sub._handle:
                        _emit_qos_warnings(
                            take_json(lib.ros_subscriber_events(sub._handle))
                        )
    ...
    def close(self) -> None:
        ...
        if self._param_server is not None:
            lib.ros_param_server_free(self._param_server)
            self._param_server = None
        if self._ctx:
            lib.ros_shutdown(self._ctx)
            self._ctx = None
