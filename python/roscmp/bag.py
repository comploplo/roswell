...
Thin ctypes shim over the Rust MCAP machinery (``roscmp-dds``'s reader/writer,
exposed through ``rcm_bag_*`` handles): chunking, lz4 (de)compression, record
parsing, and message encode/decode all run in Rust. Python only walks the
cursor and materializes results.

    import roscmp.bag

    with roscmp.bag.open_write("out.mcap") as bag:
        bag.write("/chatter", msg, timestamp_ns)

    for sample in roscmp.bag.read("out.mcap"):
...
def read(
    ...
    back as raw bytes.
    """
    handle = check_handle(lib.rcm_bag_open_read(str(path).encode()), f"read({path})")
    types: Dict[str, Optional[MessageType]] = {}
    roots = _interfaces.search_roots([str(p) for p in type_paths])
    try:
        while True:
            r = lib.rcm_bag_next(handle)
            if r == 0:
                return
            if r < 0:
                raise error_for(r, "bag read")
            info = json.loads(take_json(lib.rcm_bag_info_json(handle)))
            ros_type = info["type"]
            ...
            if mtype is not None:
                msg = mtype.alloc()
                if lib.rcm_bag_decode(handle, msg.handle) == 1:
                    message = msg
                else:
                    msg.close()
            schema = None
        ...
        lib.rcm_bag_reader_free(handle)
...
def _resolve(ros_type: str, roots: Sequence[str]) -> Optional[MessageType]:
    ...
    handle = lib.rcm_type_resolve(ros_type.encode(), arr, len(items))
    return MessageType(handle) if handle else None


def _raw_data(handle: int, size: int) -> bytes:
    buf = ctypes.create_string_buffer(size)
    n = lib.rcm_bag_data(handle, buf, size)
    if n < 0:
...
def _schema(handle: int, ros_type: str) -> Optional[str]:
    ptr = lib.rcm_bag_schema(handle, ros_type.encode())
    if not ptr:
        return None
    text = take_json(ptr)
    return text or None
...
class BagWriter:
    ...
    def __repr__(self) -> str:
        ...
        return f"<roscmp.bag.BagWriter path={self.path!r} {state}>"

    def write(self, topic: str, msg: Message, timestamp_ns: int) -> None:
        """Append ``msg`` on ``topic`` at ``timestamp_ns`` (nanoseconds)."""
        check_rc(
            lib.rcm_bag_write(self._handle, topic.encode(), timestamp_ns, msg.handle),
            f"bag write({topic})",
        )
    ...
    def close(self) -> None:
        ...
        if self._handle:
            check_rc(lib.rcm_bag_writer_close(self._handle), "bag close")
            self._handle = None
...
def open_write(path, compression: str = "lz4") -> BagWriter:
    ...
    handle = lib.rcm_bag_open_write(str(path).encode(), compression.encode())
