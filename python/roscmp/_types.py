...
The Rust core is the single source of truth for layout: ``rcm_type_layout_json``
...
``rcm_seq_assign`` / ``rcm_str_assign`` (addressed by the message handle plus a
...
class MessageType:
    ...
    def __init__(self, handle: int):
        ...
        layout = json.loads(take_json(lib.rcm_type_layout_json(handle)))
    ...
    def alloc(self) -> "Message":
        ...
        handle = lib.rcm_msg_alloc(self._handle)
        if not handle:
            raise RoscmpError("rcm_msg_alloc failed")
        ptr = lib.rcm_msg_data(handle)
...
class Message:
    ...
    def __getattr__(self, name: str):
        ...
        if object.__getattribute__(self, "_root")._closed:
            raise RoscmpError("message is closed")
    ...
    def __repr__(self) -> str:
        return f"<roscmp.Message {self._mid} at 0x{self._addr:x}>"
    ...
    def _set(self, f, value) -> None:
        ...
        if mult == "scalar":
            if kind == "prim":
                ...
                if lib.rcm_str_assign(self.handle, off, b, len(b)) != 0:
                    raise RoscmpError("rcm_str_assign failed")
            else:
                raise RoscmpError(
            ...
            if kind != "prim":
                raise RoscmpError("assignment to non-primitive arrays is unsupported")
            self._set_prim_array(el, addr, f["array_len"], value)
        else:  # sequence
            if kind != "prim":
                raise RoscmpError("assignment to non-primitive sequences is unsupported")
    ...
    def _set_prim_seq(self, el, field_offset: int, value) -> None:
        ...
        if lib.rcm_seq_assign(self.handle, off, el["size"], el["align"], src, count) != 0:
            raise RoscmpError("rcm_seq_assign failed")
