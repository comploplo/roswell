...
use roscmp::ir::MsgId;
use roscmp::workspace::{load_action_ref, load_message_ref, load_service_ref, ActionTypes};
...
use roscmp_dds::parameters::{ParameterEvent, ParameterServer, ParameterType, ParameterValue};
...
use roscmp_dds::tf::{TfBuffer, Transform as TfTransform};
use roscmp_dds::time::Time;
use roscmp_dds::transport::{Dds, DdsPub, Qos};
...
/// [`DynamicType`]. Mirrors the generic `roscmp_dds::action::ActionClient`, but
...
/// XCDR1 little-endian encapsulation header (see `roscmp::cdr`).
...
/// Re-encode the nested `field` of a decoded `wrapper` (living in `scratch`) via
/// the nested type's own layout, then decode it into caller message `out`.
unsafe fn extract_nested(
    ...
    let payload_cdr = match nested.encode(scratch.add(off)) {
        Ok(cdr) => cdr,
        Err(e) => {
            set_error(format!("action payload re-encode failed: {e}"));
            return Err(RCM_ERR_ENCODE);
        }
    };
    nested.fini(out);
    if let Err(e) = nested.decode(&payload_cdr, out) {
...
pub extern "C" fn rcm_bag_decode(bag: RcmHandle, out: RcmHandle) -> c_int {
    ...
    if roscmp_dds::raw::ros_type_to_dds_type(sample.msg.ros_type()) != m.dds_type {
        set_error(format!(
...
/// `roscmp-dds/tests/action_client_loopback.rs`, but type-blind. Goals are
