//! `roscmp-build`: generate roscmp Rust message bindings from a `build.rs`,
...
//! ```
//!
//! Then in the consuming crate (which must depend on `roscmp-dds` — the
...
use roscmp::workspace::{Kind, resolve_ref};
...
impl Config {
    ...
    /// against the configured type paths and write `roscmp_msgs.rs` — repr(C)
    /// structs, CDR (de)serializers, and `roscmp_dds` `CdrMsg`/action-trait
    ...
    pub fn compile(
        ...
        let code = externalize(&roscmp::codegen::rust::generate_dds(&program));
        std::fs::create_dir_all(&out_dir)
            .map_err(|e| Error(format!("creating {}: {e}", out_dir.display())))?;
        let path = out_dir.join("roscmp_msgs.rs");
...
/// output normally lives *inside* roscmp-dds (`msgs.rs`). Retarget them at the
/// external `roscmp_dds` crate, and route the one cross-crate value —
/// `Time::to_msg()` returns roscmp-dds's `builtin_interfaces__Time`, not the
...
fn externalize(code: &str) -> String {
    code.lines()
        ...
        .map(|line| line.replace("crate::", "::roscmp_dds::"))
...
mod tests {
    ...
    fn generates_externalized_bindings() {
        let dir = std::env::temp_dir().join(format!("roscmp_build_{}", std::process::id()));
        let path = Config::new()
        ...
        assert!(code.contains("impl ::roscmp_dds::codec::CdrMsg for geometry_msgs__Twist"));
        assert!(code.contains(
            "impl ::roscmp_dds::action::SendGoalRequest for \
    ...
    fn missing_reference_is_a_clean_error() {
        let dir = std::env::temp_dir().join(format!("roscmp_build_err_{}", std::process::id()));
        let err = Config::new()
            .type_paths([samples_root()])
            .out_dir(&dir)
