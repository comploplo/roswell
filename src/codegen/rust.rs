...
pub fn generate(program: &Program) -> String {
    generate_inner(program, false, true)
}

/// Like [`generate`], but also emits a `crate::codec::CdrMsg` impl per message so
/// the output can ride RTPS in `roswell-ros2-compat`. Not self-contained: it references
/// `crate::codec`, so it only compiles inside that crate.
pub fn generate_dds(program: &Program) -> String {
    generate_inner(program, true, true)
}

/// Like [`generate_dds`], but omits the `#[no_mangle] extern "C"` FFI exports.
/// Those are global symbols owned by the single canonical bundle inside
/// roswell-ros2-compat; a downstream crate that re-generates overlapping messages
/// would define the same symbols and collide with the runtime crate at link
/// time. Rust consumers use the `CdrMsg` trait, not the C ABI, so external
/// build-time bindings (see `roswell-build`) drop them.
pub fn generate_dds_external(program: &Program) -> String {
    generate_inner(program, true, false)
}

fn generate_inner(program: &Program, dds: bool, ffi: bool) -> String {
    ...
    for msg in &program.messages {
        ...
        emit_message(&mut out, msg, hash.as_deref(), ffi);
        if dds {
...
/// The C-ABI buffer type returned by `*_serialize`, freed by `roswell_buf_free`.
...
fn emit_message(out: &mut String, msg: &Message, type_hash: Option<&str>, ffi: bool) {
...
/// Emit the `crate::codec::CdrMsg` impl that lets `roswell-ros2-compat` transport this
...
fn emit_ffi(out: &mut String, name: &str) {
    out.push_str(&format!(
        "
#[no_mangle]
pub extern \"C\" fn roswell_{name}_serialize(msg: *const {name}, endian: u8) -> RoswellBuf {{
    if msg.is_null() {{ return RoswellBuf {{ ptr: core::ptr::null_mut(), len: 0, cap: 0 }}; }}
    let e = if endian == 0 {{ Endian::Little }} else {{ Endian::Big }};
    let v = unsafe {{ (*msg).to_cdr(e) }};
    let mut v = core::mem::ManuallyDrop::new(v);
    RoswellBuf {{ ptr: v.as_mut_ptr(), len: v.len(), cap: v.capacity() }}
}}

#[no_mangle]
pub extern \"C\" fn roswell_{name}_deserialize(buf: *const u8, len: usize, out: *mut {name}) -> i32 {{
...
#[no_mangle]
pub extern \"C\" fn roswell_{name}_fini(msg: *mut {name}) {{
