#!/usr/bin/env bash
#
# Honest head-to-head bench: roswell wheel vs rclpy, both inside ONE ros:jazzy
...
podman run --rm \
  ...
  "$IMAGE" bash -euo pipefail -c '
    ...
    pip install -q --break-system-packages /bench/roswell-*.whl
    export ROS_DOMAIN_ID=17
    echo "== rclpy =="
    python3 /bench/benchmarks/bench_rclpy.py
    echo "== roswell =="
    python3 /bench/benchmarks/bench_roswell.py
  '
