...
/// Create the `rq/`/`rr/` topic pair for `/<service>` with explicit DDS type
/// names. Shared by the typed [`Service`]/[`Client`] (type names from
/// [`CdrMsg::TYPE_NAME`]) and the runtime-typed `raw` twins (names supplied at
/// construction).
pub(crate) fn topic_pair(
    dds: &Dds,
    service: &str,
    request_type: &str,
    reply_type: &str,
) -> (rustdds::Topic, rustdds::Topic) {
...
impl<Req: CdrMsg, Resp: CdrMsg> Service<Req, Resp> {
    ...
    pub fn new(dds: &Dds, service: &str) -> Self {
        let (req_topic, reply_topic) = topic_pair(dds, service, Req::TYPE_NAME, Resp::TYPE_NAME);
        let qos = dds.qos();
        let reader = dds
            .participant()
