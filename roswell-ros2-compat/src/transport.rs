...
impl<M: CdrMsg> DdsSub<M> {
    /// Take the next sample together with its source writer's 16-byte GID (the
    /// publishing DataWriter's GUID).
    pub fn take_with_source(&mut self) -> Option<(M, [u8; 16])> {
        let sample = self.reader.take_next_sample().ok().flatten()?;
        let gid = sample.sample_info().writer_guid().to_bytes();
        Some((sample.into_value(), gid))
    }
