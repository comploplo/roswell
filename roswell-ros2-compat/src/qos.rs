...
use rustdds::{
    policy::{
        Deadline, Durability, History, Lifespan, Liveliness as DdsLiveliness, Reliability,
        ResourceLimits,
    },
...
impl QosProfile {
    ...
    pub fn policies(&self) -> QosPolicies {
        ...
        let mut b = QosPolicyBuilder::new()
            ...
            .history(history);
        // Bound the reliable writer's retransmit send buffer. When no
        // `ResourceLimits` is supplied, rustdds's memory-safety backstop falls
        // back to `DEFAULT_WRITER_MAX_SAMPLES` (8192) and its periodic cache
        // cleaner only reclaims every 6 s, so a keep-last topic whose reader is
        // absent or lagging accumulates thousands of retained samples
        // (max_samples × payload) before anything is freed — ~1.5 GB+ for 10 MB
        // samples. Keep-last already promises only the last `depth` samples, so
        // tie the writer's sample cap to `depth`; this bounds the send buffer at
        // `depth × payload` regardless of reader liveness. `KeepAll` deliberately
        // opts out (the caller asked to retain everything).
        if let History::KeepLast { depth } = history {
            b = b.resource_limits(ResourceLimits {
                max_samples: depth,
                max_instances: 1,
...
mod tests {
    ...
    #[test]
    fn keep_last_bounds_writer_resource_limits_to_depth() {
        // The retransmit-buffer cap: keep-last depth must bound max_samples so
        // an absent reader can't balloon the writer cache to 8192 samples.
        let p = QosProfile::from_preset(Qos::Default).policies();
        assert_eq!(
            p.resource_limits(),
            Some(rustdds::policy::ResourceLimits {
                max_samples: 10,
                max_instances: 1,
    ...
    #[test]
    fn keep_all_opts_out_of_resource_limits() {
        let p = QosProfile::from_preset(Qos::Default)
            .with_keep_all()
            .policies();
        assert_eq!(p.resource_limits(), None);
    }
