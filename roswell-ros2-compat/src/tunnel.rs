...
// Frame magic and kind tags live in the no_std `roswell-tunnel-core` crate so a
...
use roswell_tunnel_core::kind;
const MAGIC: &[u8; 8] = &roswell_tunnel_core::MAGIC;
const MAX_FRAME_LEN: usize = roswell_tunnel_core::MAX_PAYLOAD_LEN;
...
impl Default for TopicBridgeTxConfig {
    fn default() -> Self {
        Self {
            peer: "roswell-ros2-compat topic bridge".into(),
...
impl OutboundQueue {
    ...
    fn enqueue(&mut self, kind: ChannelKind, name: &str, frame: QueuedFrame) {
        ...
        let max_pending = frame.policy.max_pending;
        {
            ...
            // `roswell_verify::plan_enqueue`.
            let plan = roswell_verify::plan_enqueue(queue.len(), max_pending, drop_oldest);
            if plan.drop_front {
                queue.pop_front();
            }
            if plan.push {
...
mod tests {
    ...
    use super::{
        read_frame, read_frame_resync, write_frame, ChannelKind, FrameSink, FrameSource, FramedIo,
        OutboundQueue, Priority, ReliabilityMode, RoutedTopicSink, RoutedTopicSource,
        TopicBridgeRx, TopicBridgeRxConfig, TopicBridgeTx, TopicBridgeTxConfig, TopicSampleSink,
        TopicSampleSource, TunnelFrame, TunnelPolicy, TunnelReliabilityHandle,
    };
    use crate::raw::RawMsg;
