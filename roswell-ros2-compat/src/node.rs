    ...
    pub fn from_args_on_domain(name: impl Into<String>, args: &[String], domain: u16) -> Self {
        ...
        for file in &parsed.params_files {
            match std::fs::read_to_string(file) {
                ...
                Err(err) => eprintln!("roswell: ignoring --params-file '{file}': {err}"),
            }
        }
    ...
    fn tick_nanos(&mut self) -> Option<i64> {
        match self.clock.mode() {
            ClockMode::Wall => {
                ...
                // (`roswell_verify::next_fire_after`) requires `now < i64::MAX`.
...
fn parse_ros_args(args: &[String]) -> RosArgs {
    ...
    while i < args.len() {
        ...
        match arg {
            ...
            other => eprintln!("roswell: ignoring unknown --ros-args option '{other}'"),
        }
        i += 1;
    }
    out
...
fn apply_remap(out: &mut RosArgs, rule: &str) {
    let Some((from, to)) = split_assign(rule) else {
        eprintln!("roswell: ignoring malformed remap '{rule}'");
        return;
    };
    match from {
...
impl Timer {
    ...
    fn fire(&mut self, dds: Option<&Dds>, now_nanos: i64) {
        ...
        // `now_nanos < i64::MAX`. See `roswell_verify::next_fire_after`.
        self.next_fire_nanos =
            roswell_verify::next_fire_after(self.next_fire_nanos, self.period_nanos, now_nanos);
    }
}
