...
/// The key follows rosbag2's convention, but the value is roswell's own compact
...
impl<W: Write> McapWriter<W> {
    ...
    fn build(mut writer: W, chunk: Option<ChunkState>) -> io::Result<Self> {
        ...
        write_record(&mut writer, Op::Header, |payload| {
            ...
            write_string(payload, "roswell-ros2-compat")?;
            Ok(())
        })?;
        Ok(Self {
            writer,
...
impl RawDdsPublisher {
    ...
    /// Publish by serializing directly into the transport-bound buffer — the
    /// loaned-buffer publish path.
    ///
    /// `fill` receives an empty `Vec<u8>` pre-sized to `size_hint` and must write
    /// the full CDR message (4-byte encapsulation header + body) into it, exactly
    /// as [`RawMsg`]'s `cdr` holds it. That buffer is then frozen to [`Bytes`]
    /// (a zero-copy `Vec`→`Bytes` move) and handed to the DDS writer, which keeps
    /// the `Bytes` by reference in its history cache. The encapsulation-body view
    /// the transport reads is a zero-copy subslice of that same allocation (see
    /// [`RawPayload::body_bytes`]), so the serialize buffer, the stored sample,
    /// and the wire-body view all share one allocation — the only per-sample copy
    /// left is the one rustdds itself makes when fragmenting onto the socket.
    ///
    /// Returns the number of bytes written, so a caller can feed it back as the
    /// next `size_hint` and, on a same-sized sample, reallocate zero times.
    ///
    pub fn publish_loaned(&self, size_hint: usize, fill: impl FnOnce(&mut Vec<u8>)) -> usize {
...
impl RawPayload {
    ...
    /// The CDR body (payload without the 4-byte encapsulation header) as a
    /// zero-copy [`Bytes`] view sharing this payload's allocation. rustdds stores
    /// the returned `Bytes` directly in its history cache (see `DDSData` /
    /// `SerializedPayload::new_from_bytes`), so slicing here rather than copying
    /// removes one full-payload memcpy from the publish hot path — for a 10 MB
    /// sample, ~10 MB of copy per publish.
    fn body_bytes(&self) -> Bytes {
        if self.cdr.len() >= 4 && matches!(&self.cdr[..2], [0x00, 0x00 | 0x01]) {
            self.cdr.slice(4..)
        } else {
            self.cdr.clone()
        }
...
impl RawService {
    ...
    pub fn new(dds: &Dds, service: &str, request_type: &str, reply_type: &str) -> Self {
        let (req_topic, reply_topic) =
            crate::service::topic_pair(dds, service, request_type, reply_type);
        let qos = dds.qos();
        let reader = dds
            .participant()
