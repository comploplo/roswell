//! Glue between roswell's generated CDR (de)serializers and RustDDS's adapter
...
/// Error from the roswell CDR codec at the DDS boundary.
#[derive(Debug)]
pub struct CodecError(pub &'static str);
impl std::fmt::Display for CodecError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "roswell codec: {}", self.0)
    }
}
impl std::error::Error for CodecError {}

/// A message that can ride RTPS using roswell's CDR and ROS2 naming.
...
impl<T: CdrMsg> SerializerAdapter<T> for Ser<T> {
    ...
    fn to_bytes(value: &T) -> Result<Bytes, CodecError> {
        // `encode()` yields an owned `Vec` (header + body); move it into `Bytes`
        // and return a zero-copy subslice past the 4-byte encapsulation header,
        // rather than copying the body out. rustdds keeps this `Bytes` by
        // reference, so the encoded buffer is never copied again on our side.
        Ok(Bytes::from(value.encode()).slice(4..))
    }
}
