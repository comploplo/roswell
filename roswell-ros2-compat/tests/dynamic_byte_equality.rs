//! Proof that the runtime, layout-driven [`roswell::dynamic`] codec is
//! byte-for-byte identical to the *generated* `to_cdr`/`from_cdr` in
//! `roswell_ros2_compat::msgs`, operating over the **same C-ABI struct memory**.
//!
...
//! counterpart) round-trip against hand-driven `roswell::cdr::Writer` bytes.

use core::mem::{align_of, offset_of, size_of};

use roswell::cdr::{Endian, Writer};
...
fn check<T: CdrMsg>(ty: &DynamicType, mut gen: T) {
    ...
    // `encode_into` promises byte-for-byte identity with `encode` while reusing
    // the caller's buffer: hand it a dirty, over-sized buffer and compare.
    let mut reused = vec![0xAA_u8; expected.len() + 32];
    unsafe { ty.encode_into(core::ptr::from_ref(&gen).cast::<u8>(), &mut reused) };
    assert_eq!(reused, expected, "encode_into != encode");
...
fn service_loader_from_sample_srv() {
    ...
    let (req, resp) = roswell::dynamic::load_service(&path, &[] as &[&std::path::Path]).unwrap();
