...
use roswell_ros2_compat::action::{
    ActionNames, CancelGoalResponse, FibonacciActionClient, GoalId, GoalStatus, GoalStatusArrayMsg,
    GoalStatusMsg,
};
...
type FibClient = FibonacciActionClient;
...
fn run_server(stop: &Arc<AtomicBool>) {
    ...
    let mut cancel_goal = Service::<
        roswell_ros2_compat::action::CancelGoalRequest,
        CancelGoalResponse,
    >::new(&dds, &names.cancel_goal);
    let feedback = dds
...
fn action_client_goal_feedback_result_loopback() {
    ...
    let dds = Dds::new(0);
    let mut client: FibClient = FibClient::new(&dds, "/fibonacci");
...
fn action_client_cancel_reaches_server() {
    ...
    let dds = Dds::new(0);
    let mut client: FibClient = FibClient::new(&dds, "/fibonacci_cancel");
