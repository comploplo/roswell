#!/bin/bash
# M4 interop proof: every roswell-ros2-compat endpoint exercised against a *vanilla* ROS2
...
cargo build --release -p roswell-ros2-compat --bins 2>&1 | tail -2
...
check 1 "hello from roswell" /tmp/t1.txt
...
check 5 "hello from roswell" /tmp/t5.txt
...
check 6 "hello from roswell" /tmp/t6.txt
...
( timeout 28 $B/graph --advertise-node roswell_discovery_node > /tmp/t15node.txt 2>&1 ) &
sleep 6; timeout 10 ros2 node list > /tmp/t15.txt 2>&1 || true; sleep 1
check 15 "roswell_discovery_node" /tmp/t15.txt
...
timeout 10 ros2 node info /roswell_discovery_node > /tmp/t16.txt 2>&1 || true; sleep 1
