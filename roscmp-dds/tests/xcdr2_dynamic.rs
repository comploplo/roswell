...
//! [`roscmp::dynamic`] codec and the generated `to_cdr_xcdr2`/`from_cdr`.
//!
...
// (msgs.rs inlines the runtime verbatim), distinct from `roscmp::cdr::Endian`.
use roscmp::ir::MsgId;
use roscmp::{parse_message, resolve};
use roscmp_dds::msgs::Endian as MEndian;
...
fn wide_xcdr2_is_strictly_smaller() {
    ...
    let mut w = roscmp::cdr::Writer::with_encoding(roscmp::cdr::Endian::Little, Encoding::Xcdr2);
    w.write_u8(1);
    w.write_f64(2.0);
    w.write_i64(3);
    let x2 = w.finish();
