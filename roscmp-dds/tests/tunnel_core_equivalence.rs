//! Byte-for-byte equivalence between the std `roscmp_dds::tunnel` codec and the
//! `no_std` `roscmp_tunnel_core` codec that a no-DDS MCU uses.
//!
//! This is the twin-implementation pin (in the spirit of the `roscmp_verify`
...
use roscmp_dds::raw::RawMsg;
use roscmp_dds::tunnel::{read_frame, write_frame, TunnelFrame};
...
fn mcu_core_encoded_frames_parse_in_the_host() {
    ...
    let n = wire::encode_hello(&mut buf, "roscmp-mcu").unwrap();
    assert_eq!(
